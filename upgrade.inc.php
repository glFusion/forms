...
function FRM_do_upgrade($current_ver)
{
    ...
    if ($current_ver < '0.0.5') {
        ...
        $error = FRM_upgrade_0_0_5();
        if ($error)
            return $error;
    }

    if ($current_ver < '0.1.0') {
        COM_errorLog("Updating Plugin to 0.1.0");
        $error = FRM_upgrade_0_1_0();
        if ($error)
            return $error;
    }

    if ($current_ver < '0.1.1') {
        COM_errorLog("Updating Plugin to 0.1.1");
        $error = FRM_upgrade_0_1_1();
        if ($error)
            return $error;
    }

    if ($current_ver < '0.1.2') {
        COM_errorLog("Updating Plugin to 0.1.2");
        $error = FRM_do_upgrade_sql('0.1.2');
        if ($error) return $error;
    }

    if ($current_ver < '0.1.3') {
        COM_errorLog("Updating Plugin to 0.1.3");
        $error = FRM_do_upgrade_sql('0.1.3');
        if ($error) return $error;
    }

    if ($current_ver < '0.1.5') {
        COM_errorLog("Updating Plugin to 0.1.5");
        $error = FRM_do_upgrade_sql('0.1.5');
        if ($error) return $error;
    }

    if ($current_ver < '0.1.6') {
        COM_errorLog("Updating Plugin to 0.1.6");
        // Just one query, adding the unique key.  It may have been added
        // previously by testers, so ignore any error
        FRM_do_upgrade_sql('0.1.6');
    }

    if ($current_ver < '0.1.7') {
        COM_errorLog("Updating Plugin to 0.1.7");
        $error = FRM_upgrade_0_1_7();
        if ($error) return $error;
    }

    if ($current_ver < '0.1.8') {
        COM_errorLog("Updating Plugin to 0.1.8");
        // Only SQL upgrade in this version
        $error = FRM_do_upgrade_sql('0.1.8');
        if ($error) return $error;
    }

    if ($current_ver < '0.2.0') {
        COM_errorLog("Updating Plugin to 0.2.0");
        // Only SQL upgrade in this version
        $error = FRM_do_upgrade_sql('0.2.0');
        if ($error) return $error;
    }

    if ($current_ver < '0.2.2') {
        COM_errorLog("Updating Plugin to 0.2.2");
        // Only SQL upgrade in this version
        $error = FRM_do_upgrade_sql('0.2.2');
        if ($error) return $error;
    }
...
*/
function FRM_do_upgrade_sql($version = 'Undefined', $sql='')
{
    ...
    if ($version == 'Undefined') {
        COM_errorLog("Error updating {$_CONF_FRM['pi_name']} - Undefined Version");
        return 1;
    }
...
function FRM_upgrade_0_0_5()
{
    ...
    $status = FRM_do_upgrade_sql('0.0.5');
    ...
    DB_query("DROP TABLE {$_TABLES['forms_frmXfld']}");
...
function FRM_upgrade_0_1_0()
{
    ...
    $status = FRM_do_upgrade_sql('0.1.0');
    if ($status) return $status;
...
function FRM_upgrade_0_1_7()
{
    ...
    $error = FRM_do_upgrade_sql('0.1.7');
    if ($error) return $error;
